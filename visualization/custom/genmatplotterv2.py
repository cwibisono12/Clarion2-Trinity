...
class genmatplotter:
	...
	C. Wibisono
	10/17 '23
	#v2 04/09 '24 more stable version
	...
	def process(self,rebinfactor):
		...
		if int(self.gate[0][0]) == 1:
			self.x = r.rebin(int(self.matlist[0][3]),self.project[0],rebinfactor).rebinx()
		else:
			self.x = r.rebin(int(self.matlist[0][2]),self.project[0],rebinfactor).rebinx()
	...
	def plot(self,xlow,xup,title):
		...
		#Transfrom xlow and xup in terms of xrebin,yrebin coordinates space:
		...
		for n in range(iterator):
			if self.flagad == 0:
				if iterator > 1: #temp
					lineplotind,=self.ax[n].plot(self.x[xlowtrf:xuptrf],self.projectrebin[n][xlowtrf:xuptrf],linewidth=0.85,ls='steps-mid',color=self.colorlist[n],label=self.labelnonad[n])
				else:
					lineplotind,=self.ax.plot(self.x[xlowtrf:xuptrf],self.projectrebin[n][xlowtrf:xuptrf],linewidth=0.85,ls='steps-mid',color=self.colorlist[n],label=self.labelnonad[n])

			if self.flagad == 1:
				lineplotind,=self.ax[n].plot(self.x[xlowtrf:xuptrf],self.projectrebin[n][xlowtrf:xuptrf],linewidth=0.85,ls='steps-mid',color=self.colorlist[n],label=self.labelad[n])
	...
	def update(self,event):
		...
		#Transfrom xlow and xup corrdinates into xrebin and yrebin coordinate space:
		xlowtrf = np.argmin(np.abs(self.x - xlow))
		xuptrf = np.argmin(np.abs(self.x - xup))
