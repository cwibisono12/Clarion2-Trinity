...
void *write_data4dyn_th(void *arguments){

ARGPARSE4 *parameters = (ARGPARSE4*) arguments;
int xdim=parameters->xdim;
int ydim=parameters->ydim;
char *filename=parameters->filename;
int overwrite=parameters->overwrite;
int k;
int *data[4096];

for(k=0;k<4096;k++){
data[k] =parameters->data[k];
}
    FILE *FP;
    int i,j;
    int *previous[ydim];
