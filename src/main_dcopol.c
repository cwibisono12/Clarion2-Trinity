...
//  int Emax=atof(argv[7]); //C.W
  ...
  for(bc=0;bc<5000;bc++){
	if((gg_prompt_puref[bc]=(int*)malloc(5000*sizeof(int))) == NULL){
	printf("\nError,memory not allocated\n");
	exit(1);
	}   
	if((gg_prompt_pureb[bc]=(int*)malloc(5000*sizeof(int))) == NULL){
	printf("\nError,memory not allocated\n");
	exit(1);
	}   
	if((gg_nprompt_puref[bc]=(int*)malloc(5000*sizeof(int))) == NULL){
	printf("\nError,memory not allocated\n");
	exit(1);
	}   
	if((gg_nprompt_pureb[bc]=(int*)malloc(5000*sizeof(int))) == NULL){
	printf("\nError,memory not allocated\n");
	exit(1);
	}   
	if((gg_prompt_purefp[bc]=(int*)malloc(5000*sizeof(int))) == NULL){
	printf("\nError,memory not allocated\n");
	exit(1);
	}   
	if((gg_prompt_purebp[bc]=(int*)malloc(5000*sizeof(int))) == NULL){
	printf("\nError,memory not allocated\n");
	exit(1);
	}   
	if((gg_nprompt_purefp[bc]=(int*)malloc(5000*sizeof(int))) == NULL){
	printf("\nError,memory not allocated\n");
	exit(1);
	}   
	if((gg_nprompt_purebp[bc]=(int*)malloc(5000*sizeof(int))) == NULL){
	printf("\nError,memory not allocated\n");
	exit(1);
	}   
...
//Generate DCO Matrices:
dco(gmult,gaggvalid,ge);

//Generate POL Matrices:
pol(gmult,gaggvalid,ge);
...
//printf("Ep<200: %d,Ep>200: %d\n",plt200,pgt200);
  ...
  //Event Spectra   
  write_data4dyn(GG_PROMPT_PUREF,gg_prompt_puref,5000,5000,overwrite);
  write_data4dyn(GG_PROMPT_PUREB,gg_prompt_pureb,5000,5000,overwrite);
  write_data4dyn(GG_NPROMPT_PUREF,gg_nprompt_puref,5000,5000,overwrite);
  write_data4dyn(GG_NPROMPT_PUREB,gg_nprompt_pureb,5000,5000,overwrite);
  write_data4dyn(GG_PROMPT_PUREFP,gg_prompt_purefp,5000,5000,overwrite);
  write_data4dyn(GG_PROMPT_PUREBP,gg_prompt_purebp,5000,5000,overwrite);
  write_data4dyn(GG_NPROMPT_PUREFP,gg_nprompt_purefp,5000,5000,overwrite);
  write_data4dyn(GG_NPROMPT_PUREBP,gg_nprompt_purebp,5000,5000,overwrite);
  
  write_data4(POL_PARA, *pol_para, 1, 5000,overwrite);
  write_data4(POL_PERP, *pol_perp, 1, 5000,overwrite);
  write_data4(POL_PARA_P, *pol_para_p, 1, 5000,overwrite);
  write_data4(POL_PERP_P, *pol_perp_p, 1, 5000,overwrite);
