...
void write_data4dyn3(char *filename, int **data[], int xdim, int ydim, int zdim, int overwrite){
    FILE *FP;
    int i,j,k;
    int **previous[zdim];
	...
	for(i=0;i<zdim;i++){
        //allocate memory for 1d-array for reading in rows of 2d Radware matrix
        	if ( ( previous[i] = (int **)malloc(ydim *  sizeof(int*)) ) == NULL ) {
            		printf("\nError, memory not allocated.\n");
            		exit(1);
        			}
		for(j=0;j<ydim;j++){
        if ( ( previous[i][j] = (int *)malloc(xdim *  sizeof(int)) ) == NULL ) {
            printf("\nError, memory not allocated.\n");
            exit(1);
        							}
      				}
        ...
        if( (FP=fopen(filename, "r")) != NULL ){
           for(i=0;i<zdim;i++){
		for(j=0;j<ydim;j++){ 
	   fread(previous[i][j], sizeof(int)*xdim, 1, FP);        
            			}
		}
	    fclose(FP);
            //update spectra
	...
	for (j=0; j<ydim; j++) {
	   for(k=0;k<xdim;k++){
                if(previous[i][j][k] < (powf(2,sizeof(int)*8.0)-2)) 
                    data[i][j][k] = data[i][j][k] + previous[i][j][k];
            			}   
        		} 
		}
	}
  ...
  //printf("hello memory2\n");
    ...
    for(i=0;i<zdim;i++){
	for(j=0;j<ydim;j++){
    		fwrite(data[i][j], sizeof(int)*xdim,1 , FP);
    			}
		}
    fclose(FP);    
}
